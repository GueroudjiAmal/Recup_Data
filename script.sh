...
LIBDARSHAN="/home/agueroudji/spack/opt/spack/linux-sles15-zen3/gcc-12.2.0/darshan-runtime-3.4.2-3dxwvjki4wo4gce4sux532uaqnzkgjeu/lib/libdarshan.so"
...
echo nprocs $NPROC, ranks $NRANKS, depth $NDEPTH
DARSHAN_ENABLE_NONMPI=1 DARSHAN_CONFIG_PATH="config.txt" LD_PRELOAD=$LIBDARSHAN  mpiexec  -n ${NPROC} --ppn ${NRANKS}  -d ${NDEPTH}  --hostfile WorkerNodes --exclusive --cpu-bind depth  dask worker  --scheduler-file=$SCHEFILE 1>> worker.o 2>>worker.e  &
     
# Wait for the client process to be finished 
wait $client_pid
wait 

mv *.darshan $PBS_O_WORKDIR/Reports
for fd in $PBS_O_WORKDIR/Reports/*.darshan; do
	LD_PRELOAD=$LIBDARSHAN python -m darshan summary $fd
done
